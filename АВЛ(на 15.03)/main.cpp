//не помню, проходили ли мы с вами операцию ?:, но здесь будет лучше ее использовать
...
private:
	...
	int bfactor( Node* node ){ // баланс(разница высот)
	...
	Node* rotateRight( Node* node ){ //повороты
		Node* tmp = node->left;
		node->left = tmp->right;
		tmp->right = node;
		fixHeight(node);
	...
	Node* remove( Node* head, int key ){ //key - значение,которое вставляем
	  if (!head)
		return NULL;
      if (key < head->key)
